...
def get_perm_cases_latest_month_data(conn) -> List[PermCaseActivityData]:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
            ...
            # Use April 2025 as the featured month for dashboard consistency
            # This provides stable reporting regardless of daily processing variations
            busiest_month = 4  # April
            busiest_year = 2025
            print(f"🔍 Using April {busiest_year} (month {busiest_month}) as featured month for dashboard")
